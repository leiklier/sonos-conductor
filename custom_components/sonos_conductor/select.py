"""Conductor selects: the TV-solo mode and the follow mode."""
...
class SonosConductorFollowModeSelect(ConductorEntity, SelectEntity, RestoreEntity):
    """The engine's follow mode (rule 1.9), restored across restarts.

    The engine seeds ``follow_mode`` to PER_ZONE; if a valid mode was
    restored, it is pushed back through the controller queue as a
    ``SetFollowMode`` event (drained after startup reconciliation, like any
    user command). An unknown or invalid restored state leaves the default.
    """

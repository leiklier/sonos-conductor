...
class ConductorEngine:
    ...
    def _seed(self, snapshot: InitialSnapshot) -> None:
        ...
        # First pass: record inputs. Every docked zone starts IDLE (undocked
        # = STANDALONE). Effective occupancy depends on the follow mode (rule
        # 1.9), which couples zones, so audibility is decided in a second pass
        # once every zone's occupancy is in place.
        for zone in self.config.zones:
            state.zones[zone.zone_id] = ZoneState(
                phase=ZonePhase.IDLE
                if state.speakers[zone.speaker_id].docked
                else ZonePhase.STANDALONE,
        ...
        # Second pass: a docked zone with effective occupancy seeds ACTIVE and
        # starts its episode at the current activity (rules 1.7, 9.1). No hold
        # timers pending at startup, so a zone without it stays IDLE.
        for zone in self.config.zones:
            zone_state = state.zones[zone.zone_id]
            if zone_state.phase is ZonePhase.IDLE and zones.follow_occupied(self, zone):

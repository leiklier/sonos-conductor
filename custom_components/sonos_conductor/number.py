...
Trim numbers adjust per-speaker loudness compensation at runtime. The master
volume itself is not a separate number: it lives on the master media player
(``media_player.sonos_conductor``), whose volume slider is the master — the
same control HomeKit and the multimedia dashboard card already surface.
"""
